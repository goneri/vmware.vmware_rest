...
async def update_changed_flag(response, operation):
    data = await response.json()

    print(data)
    if operation == "create" and response.status == 201:
        data["failed"] = False
        data["changed"] = True
    elif operation == "delete" and response.status == 204:
        data["failed"] = False
        data["changed"] = True
        ...
        # NOTE: this is a shortcut. We get this issue if a CDROM already exists
        data["failed"] = False
        data["changed"] = False
