...
def prepare_argument_spec():
    ...
    argument_spec["state"] = {"type": "str", "choices": ["find", "update"]}
    argument_spec["name"] = {"type": "str", "operationIds": ["find"]}
    argument_spec["library_id"] = {"type": "str", "operationIds": ["update"]}
...
def url(params):
    return "https://{vcenter_hostname}/rest/com/vmware/content/library".format(**params)
...
async def _find(params, session):
    ...
    for i in accepted_fields:
        if params[i]:
            spec[i] = params[i]
    _url = "https://{vcenter_hostname}/rest/com/vmware/content/library".format(**params)
    async with session.post(_url, json={"spec": spec}) as resp:
        try:
            ...
            _json = {}
        if ("find" == "create") and ("value" in _json):
...
async def _update(params, session):
    ...
    for i in accepted_fields:
        if params[i]:
            spec[i] = params[i]
    _url = "https://{vcenter_hostname}/rest/com/vmware/content/library/id:{library_id}".format(
        **params
    )
