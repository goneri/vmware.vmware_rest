...
def url(params):
    return "https://{vcenter_hostname}/rest/vcenter/vcha/cluster/witness".format(
        **params
    )
...
async def _check(params, session):
    ...
    for i in accepted_fields:
        if params[i]:
            spec[i] = params[i]
    _url = "https://{vcenter_hostname}/rest/vcenter/vcha/cluster/witness?action=check".format(
        **params
    )
...
async def _redeploy(params, session):
    accepted_fields = ["placement", "vc_spec", "ha_ip"]
    if "redeploy" == "create":
        _exists = await exists(params, session)
        if _exists:
            return await update_changed_flag({"value": _exists}, 200, "get")
    ...
    for i in accepted_fields:
        if params[i]:
            spec[i] = params[i]
    _url = "https://{vcenter_hostname}/rest/vcenter/vcha/cluster/witness".format(
        **params
    )
    async with session.post(_url, json={"spec": spec}) as resp:
        try:
            ...
            _json = {}
        if ("redeploy" == "create") and ("value" in _json):
            if type(_json["value"]) == dict:
                _id = list(_json["value"].values())[0]
            else:
                _id = _json["value"]
