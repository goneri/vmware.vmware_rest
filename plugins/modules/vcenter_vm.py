...
async def _register(params, session):
    accepted_fields = ["placement", "datastore", "datastore_path", "name", "path"]
    if "register" == "create":
        _exists = await exists(params, session)
        if _exists:
            return await update_changed_flag({"value": _exists}, 200, "get")
